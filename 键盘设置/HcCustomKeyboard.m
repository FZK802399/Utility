...
-(void)textViewShowView:(UIViewController *)viewController customKeyboardDelegate:(id)delegate
{
    ...
    UIImageView *write=[[UIImageView alloc]initWithFrame:CGRectMake(10, 15, 18, 20)];
    write.image=[UIImage imageNamed:@"xie图标.png"];
    [self.mBackView addSubview:write];
    ...
    self.mTalkBtn.frame =CGRectMake(write.right+5, 16, 30, 18);
    [self.mTalkBtn setTintColor:[UIColor blackColor]];
    [self.mBackView addSubview:self.mTalkBtn];
    
    self.mSecondaryBackView =[[UIView alloc]initWithFrame:CGRectMake(self.mTalkBtn.right+5, 10, self.mViewController.view.frame.size.width-130, 30)];
    ...
    self.mTextView =[[UITextView alloc]initWithFrame:CGRectMake(1, 1, self.mSecondaryBackView.width-2, 28)];
    ...
    self.mTextView.layer.borderColor=[UIColor colorWithRed:110.0/255.0 green:110.0/255.0 blue:110.0/255.0 alpha:0.3].CGColor;
    ...
    self.mSendBtn =[UIButton buttonWithType:UIButtonTypeRoundedRect];
    [self.mSendBtn setTitle:@"发送" forState:UIControlStateNormal];
    [self.mSendBtn addTarget:self action:@selector(forTalk) forControlEvents:UIControlEventTouchUpInside];
    self.mSendBtn.frame =CGRectMake(self.mSecondaryBackView.right+5, 16, 30, 18);
    [self.mSendBtn setTintColor:[UIColor blackColor]];
    [self.mBackView addSubview:self.mSendBtn];
...
{    
    self.isTop =NO;
        ...
        self.mHiddeView.alpha =0.0f;
        self.mSecondaryBackView.frame =CGRectMake(self.mTalkBtn.right+5, 10, self.mViewController.view.frame.size.width-130, 30);
    } completion:^(BOOL finished) {
